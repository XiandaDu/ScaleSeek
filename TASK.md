...
- 正式实验禁止 `-n/--n`、`--offset`、不可复现的随机抽样或只跑 first-N。
  **唯一豁免**是 DCI/DR-DCI/RISE 的「评测规模上限」（见第二阶段该节）：上限 1,500 题、
  由 id 的 SHA256 确定性选出、带 manifest 强制校验。任何做不到"可复现+可校验"的
  缩规模仍然禁止——被否定的是 seed 不可考，不是规模本身。
...
### 评测规模上限（2026-07-30 用户批准，唯一豁免）

**仅 DCI、DR-DCI、RISE 三个方法**在每个数据集上评测**最多 1,500 题**；其余全部方法一律完整 split。

规则是**绝对上限，不是比例**：`n_eval = min(数据集全量, 1500)`。
数据集本身 ≤1,500 的**完整跑**，不做任何抽取。

| 数据集 | 全量 | 这三个方法跑 |
|---|---:|---|
| popqa | 14,267 | 1,500 |
| triviaqa | 11,313 | 1,500 |
| 2wikimultihopqa | 9,322 | 1,500 |
| hotpotqa | 7,405 | 1,500 |
| nq | 3,610 | 1,500 |
| musique | 2,417 | 1,500 |
| browsecomp_plus | 830 | **830（全量）** |
| bamboogle | 125 | **125（全量）** |

**为什么是上限而不是比例**：比例在小数据集上失效——1/10 的 bamboogle 只剩 13 题
（EM 的 95% 置信带约 ±27 点，等于没测），1/10 的 BCP 只剩 83 题。上限规则下这两个
**反而完整跑**（它们小，所以便宜），而六个大集被封顶。

**动机（算术问题，非调参问题）**：这三者分别是 300 / 300 / 100 次模型调用**每题**。
实测锚点 `dcilite30`：n=500 在并发 2 下耗时 26h53m，外推到 14,267 约 **32 天/行**，
而 QOS 只有 2 个作业槽、且 Phase 3 要在 8 个数据集上重跑同样矩阵。

**子集如何定义（这是本例外能成立的前提）**：

- 是**无偏抽样**，但由 **id 的纯函数**抽出，不用 RNG：
  `rank = SHA256("{dataset}:{id}")` 排序后取前 `min(N, 1500)` 条。
  SHA256 对 id 是良好的伪随机函数，所以统计上就是均匀随机抽样——只是**同时可重建**。
- 因此**无需 seed、无需状态文件**；与磁盘行序无关，重新下载或重新转换结果不变。
- 实现：`eval/subsets.py`；生成：`scripts/make_dcilite_datasets.py --datasets <ds> --cap 1500`。
- 产物 `<ds>_cap1500.manifest.json` 记录 `subset_ids_sha256`；
...
> （见 `reports/baselines.md` 顶部 legacy 横幅）。规模本身不是问题，**不可复现才是**。
> 允许缩规模的**唯一**理由是子集可复现、可校验、可审计；做不到这三点的缩规模仍然禁止。

**汇报纪律（强制）**：

- 每个 metrics 文件都带 `eval_scope` 字段（`full_split` 或 `capped_1500`），
  由 `compute_metrics.py` 自动写入，不得手工填。数据集 ≤1,500 时该字段仍是
  `full_split`——因为它确实跑满了。
- 主表中被封顶的行**必须显式标注 n**，不得与全量行并列后不加说明。
- n=1,500 时 EM 的 95% 置信带约 **±2.5 个点**；跨 scope 的差距小于该带宽时
  **不得**表述为方法差异。引用 `em_ci95` / `f1_ci95` 字段。
- 被封顶的行不参与"完整 split"这一措辞。
