...
| search_o1 | .248/.345 | .539/.604 | .233/.309 | .277/.335 | .074/.144 | .344/.437 |
...
- **BCP 域：E5 没赢**（.1286 vs BM25 .1365），而**同一个 BM25 换参数**
  （k1 25/1.0 → 1.2/0.75）掉 **9 个 F1 点**——比换检索器的影响大一个数量级

→ **"检索器轴"不是普适结论，它在短段落域成立、在长文档域让位于 BM25 参数轴。**
报告和论文里凡是引用"E5 > BM25"的地方都必须限定域。（agent 侧 E5 仍有效：
...
- **top-k=3 在三个难度档上全部最好**（+2.0 / +1.4 / +3.4 EM）。单集看都在 n=500 的
  ±2 EM 噪声内，但 3/3 同向值得当线索。**已在 n=1500 上跑验证**（job 7201/7202）。
...
| **search_o1** | 34 | 20 | **+14** | .076 | 疑似被截断，n=1500 复核中 |
| **dci** | 27 | 15 | **+12** | .088 | 疑似被截断，n=1500 复核中 |

前四个**净变化恰好为 0**、不一致题仅 5-7 个（纯采样抖动）——外推成立。
但 search_o1 和 dci 不成立，两者都是"生成长"的 agent，2048 很可能真的截断了它们。
p≈0.08 尚不能下结论，**已在 n=1500 上复核**（job 7201）。

### 10h. 官方 dci-agent-lite 放大到 n=500（#15-3，进行中）
`dci` 是唯一 D 级（全套自实现）的 baseline，需要官方实现作为效度锚点。从 n=50 放大到
n=500 后：**151/408 = .3701**（LLM judge acc），但**覆盖率只有 408/500**——92 题因
harness 基建故障无有效结果，正在补跑（job 7203）。**此数在补齐前不得写入主表**。

三轮故障的根因最终查明（见 10i③），前两轮的诊断（"节点争抢"、"降并发"）都只碰到表象。
...
工具 stdout 写到 `join(os.tmpdir(), "pi-bash-<id>.log")`，每题 grep 14GB 语料、
单个日志能到 7.6GB。739 个残留日志占了 **101GB 内存**，节点 available 从 229G 掉到 128G。
内存枯竭后伪装成两种毫不相干的报错：内核分配不出 socket 缓冲 → `write ENOBUFS`；
tmpfs 写失败 → errno **-122** → 工具输出 JSON 腰斩 → `Unterminated string`。
**识破线索是截断位置从 25 到 10931 随机分布**（固定缓冲不会这样）。
